...
public class PermissionConfiguration : IEntityTypeConfiguration<Permission>
{
    public void Configure(EntityTypeBuilder<Permission> builder)
    {
        ...
        builder.HasMany(r => r.Roles).WithMany(p => p.Permissions).UsingEntity<RolePermission>(
            l => l.HasOne<Role>().WithMany().HasForeignKey(e => e.RoleId),
            r => r.HasOne<Permission>().WithMany().HasForeignKey(e => e.PermissionId)
        );

        var a = Enum.GetValues<PermissionEnum>();

        IEnumerable<Permission> permissions = Enum.GetValues<PermissionEnum>().Select(
            permissionEnum =>
                new Permission
                {
